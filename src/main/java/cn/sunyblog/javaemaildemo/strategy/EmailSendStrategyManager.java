...
@Slf4j
@Component
public class EmailSendStrategyManager {
    
    @Resource
    private DefaultEmailSendStrategy defaultStrategy;
    
    @Resource
    private BatchEmailSendStrategy batchStrategy;
    
    @Resource
    private HighPriorityEmailSendStrategy highPriorityStrategy;
    
    /**
     * 策略注册表，按优先级排序
     */
    private final List<EmailSendStrategy> strategies = new ArrayList<>();
     ...
     * 注册策略
     */
    public void registerStrategy(EmailSendStrategy strategy) {
     ...
     * 智能选择发送策略并发送邮件
     */
    public SendResult sendEmail(List<String> toList, List<String> ccList, List<String> bccList,
                               String subject, String content, boolean isHtml, List<File> attachments) {
        
        EmailSendStrategy selectedStrategy = selectStrategy(toList, ccList, bccList, subject, content, isHtml, attachments);
        
        log.info("选择策略: {} 发送邮件", selectedStrategy.getStrategyName());
        
        long startTime = System.currentTimeMillis();
        SendResult result = selectedStrategy.send(toList, ccList, bccList, subject, content, isHtml, attachments);
     ...
     * 使用指定策略发送邮件
     */
    public SendResult sendEmailWithStrategy(String strategyName, List<String> toList, List<String> ccList, 
                                           List<String> bccList, String subject, String content, 
                                           boolean isHtml, List<File> attachments) {
        
        EmailSendStrategy strategy = findStrategyByName(strategyName);
        if (strategy == null) {
        ...
        long startTime = System.currentTimeMillis();
        SendResult result = strategy.send(toList, ccList, bccList, subject, content, isHtml, attachments);
        long duration = System.currentTimeMillis() - startTime;
     ...
     * 智能选择发送策略
     */
    private EmailSendStrategy selectStrategy(List<String> toList, List<String> ccList, List<String> bccList,
                                           String subject, String content, boolean isHtml, List<File> attachments) {
     ...
     * 根据名称查找策略
     */
    private EmailSendStrategy findStrategyByName(String strategyName) {
     ...
     * 更新策略使用统计
     */
    private void updateStatistics(String strategyName, SendResult result, long duration) {
        // 更新使用次数
     ...
     * 获取所有已注册的策略
     */
    public List<EmailSendStrategy> getAllStrategies() {
    ...
    public static class StrategyPerformance {
        ...
        public double getAverageDuration() { return totalExecutions > 0 ? (double) totalDuration / totalExecutions : 0.0; }
