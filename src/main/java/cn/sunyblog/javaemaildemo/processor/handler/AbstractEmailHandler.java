...
public abstract class AbstractEmailHandler {
    ...
    public AbstractEmailHandler(String handlerName) {
     ...
     * @return 处理结果
     */
    public final EmailHandlerResult handle(EmailContext context) {
        if (!enabled) {
        ...
        try {
            ...
            log.error("处理器 {} 处理邮件异常: {}", handlerName, e.getMessage(), e);
            ...
            return EmailHandlerResult.failure(handlerName, e.getMessage());
        }
    }
     ...
     * 异步处理邮件
     */
    private EmailHandlerResult handleAsync(EmailContext context) {
     ...
     * @return 是否继续处理
     */
    protected boolean preHandle(EmailContext context) {
        return true;
    }
     ...
     * @return 是否应该处理
     */
    protected abstract boolean shouldHandle(EmailContext context);
     ...
     */
    protected abstract void doHandle(EmailContext context) throws Exception;
     ...
     */
    protected void postHandle(EmailContext context, boolean success) {
        // 默认空实现
    }
     ...
     * @param error 异常信息
     */
    protected void handleError(EmailContext context, Exception error) {
