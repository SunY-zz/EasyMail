...
import cn.sunyblog.easymail.util.SSLTrustUtil;
...
public class MailServerConnector {
    ...
    public Store connectToServer(Session session) throws MessagingException {
        ...
        try {
            ...
            EmailException connectionEx = EmailExceptionHandler.wrapMessagingException(e, "连接邮件服务器失败");
    ...
    private void closeQuietly(Store store) {
        try {
            ...
            EmailException connectionEx = EmailExceptionHandler.wrapException(e, "关闭连接时出错");
    ...
    public Folder openInbox(Store store) throws MessagingException {
        try {
            ...
            EmailException connectionEx = EmailExceptionHandler.wrapMessagingException(e, "打开邮件文件夹失败");
            log.error(connectionEx.getFullErrorMessage(), connectionEx);
            throw connectionEx;
        }
    }
    ...
    public void closeConnection(Folder folder, Store store) {
        try {
            ...
            EmailException connectionEx = EmailExceptionHandler.wrapException(e, "重新连接失败");
            log.error(connectionEx.getFullErrorMessage(), connectionEx);
        }
    }
