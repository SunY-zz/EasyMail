...
import cn.sunyblog.easymail.mail.MailContentParser;
...
public class EmailContextBuilder {
     ...
     * @return 邮件上下文
     */
    public EmailContext buildContext(Message message, String attachmentDir) {
        try {
            EmailContext.EmailContextBuilder builder = EmailContext.builder();
            ...
            EmailProcessException processEx = EmailProcessException.parsingError("构建邮件上下文失败", e);
     ...
     * 解析基本信息
     */
    private void parseBasicInfo(Message message, EmailContext.EmailContextBuilder builder)
     ...
     * 解析附件
     */
    private void parseAttachments(Message message, EmailContext.EmailContextBuilder builder)
            throws Exception {

        List<EmailContext.AttachmentInfo> attachments = new ArrayList<>();
    ...
    private void extractAttachmentsFromMultipart(Multipart multipart,
                                                 List<EmailContext.AttachmentInfo> attachments)
            throws Exception {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (disposition != null && disposition.equalsIgnoreCase(BodyPart.ATTACHMENT)) {
                ...
                if (fileName != null) {
                    ...
                    EmailContext.AttachmentInfo attachmentInfo = EmailContext.AttachmentInfo.builder()
                            .fileName(fileName)
     ...
     * 提取标签
     */
    private void extractTags(Message message, EmailContext.EmailContextBuilder builder)
            throws MessagingException {
     ...
     * 解析时间信息
     */
    private void parseTimeInfo(Message message, EmailContext.EmailContextBuilder builder)
            throws MessagingException {
     ...
     * 解析其他属性
     */
    private void parseOtherProperties(Message message, EmailContext.EmailContextBuilder builder)
            throws MessagingException {
