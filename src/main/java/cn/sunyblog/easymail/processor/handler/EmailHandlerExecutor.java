...
public class EmailHandlerExecutor {
     ...
     * @return 处理结果
     */
    public boolean executeHandlers(EmailContext context) {
        List<EmailHandlerInfo> matchingHandlers = handlerRegistry.getMatchingHandlers(context);
        ...
        for (EmailHandlerInfo handlerInfo : matchingHandlers) {
            try {
                ...
                EmailProcessException processEx = EmailProcessException.processingError("执行邮件处理器失败", e);
     ...
     * @return 执行结果
     */
    private boolean executeHandlerSync(EmailHandlerInfo handlerInfo, EmailContext context) {
        ...
        try {
            ...
            long duration = System.currentTimeMillis() - startTime;
            EmailProcessException processEx = EmailProcessException.processingError("同步执行邮件处理器失败", e);
     ...
     * @param context     邮件上下文
     */
    private void executeHandlerAsync(EmailHandlerInfo handlerInfo, EmailContext context) {
        CompletableFuture.runAsync(() -> {
            ...
            try {
                ...
                long duration = System.currentTimeMillis() - startTime;
                EmailProcessException processEx = EmailProcessException.processingError("异步执行邮件处理器失败", e);
     ...
     * @return 调用结果
     */
    private Object invokeHandler(EmailHandlerInfo handlerInfo, EmailContext context) {
        ...
        try {
            ...
            EmailProcessException processEx = EmailProcessException.processingError("调用处理器方法失败", e);
            log.error("调用处理器方法失败: {}.{} - {}",
     ...
     * @return 处理结果
     */
    public boolean executeHandlersByGroup(EmailContext context, String group) {
        List<EmailHandlerInfo> groupHandlers = handlerRegistry.getHandlersByGroup(group);
        ...
        for (EmailHandlerInfo handlerInfo : groupHandlers) {
            if (handlerInfo.matches(context)) {
                try {
                    ...
                    EmailProcessException processEx = EmailProcessException.processingError("执行组中的邮件处理器失败", e);
