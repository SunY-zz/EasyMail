...
public class MailSender {
    ...
    private boolean sendEmail(String to, String cc, String bcc, String subject, String content, boolean isHtml, List<File> attachments) {
        try {
            // 验证邮箱地址
            if (!cn.sunyblog.easymail.exception.EmailExceptionHandler.isValidEmail(to)) {
                throw cn.sunyblog.easymail.exception.EmailValidationException.invalidEmailAddresses(Collections.singletonList(to));
            }
    ...
    public boolean sendEmailInternal(String to, String cc, String bcc, String subject, String content, boolean isHtml, List<File> attachments) throws MessagingException, UnsupportedEncodingException {
        try {
            ...
            throw cn.sunyblog.easymail.exception.EmailExceptionHandler.wrapMessagingException(e, "发送邮件到: " + to);
        } catch (UnsupportedEncodingException e) {
            log.error("邮件编码失败: 收件人={}, 主题={}, 错误={}", to, subject, e.getMessage(), e);
            throw cn.sunyblog.easymail.exception.EmailExceptionHandler.wrapIOException(e, "邮件编码");
        } catch (Exception e) {
            log.error("邮件发送异常: 收件人={}, 主题={}, 错误={}", to, subject, e.getMessage(), e);
            throw cn.sunyblog.easymail.exception.EmailExceptionHandler.wrapException(e, "发送邮件");
        }
    }
}