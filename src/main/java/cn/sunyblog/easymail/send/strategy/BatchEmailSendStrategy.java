...
@Slf4j
@Component
public class BatchEmailSendStrategy implements EmailSendStrategy {
    ...
    public SendResult send(List<String> toList, List<String> ccList, List<String> bccList,
        ...
        try {
            ...
            return SendResult.failure(toList, subject, e.getMessage(), "BATCH_SEND_ERROR", getStackTrace(e), duration);
        }
    }
    ...
    private SendResult sendBatch(List<String> toList, List<String> ccList, List<String> bccList,
        ...
        CompletableFuture<Void>[] futures = toList.stream()
                .map(to -> CompletableFuture.runAsync(() -> {
                    try {
                        // 为每个收件人单独发送
                        SendResult result = defaultStrategy.send(
        ...
        // 构建批量发送结果
        SendResult result = SendResult.batchResult(batchResults, subject, duration);
     ...
     * @return 发送结果
     */
    public SendResult sendInBatches(List<String> toList, List<String> ccList, List<String> bccList,
                                    String subject, String content, boolean isHtml,
        ...
        for (int i = 0; i < toList.size(); i += batchSize) {
            ...
            SendResult batchResult = send(batch, ccList, bccList, subject, content, isHtml, attachments);
        ...
        long duration = System.currentTimeMillis() - startTime;
        return SendResult.batchResult(allResults, subject, duration);
    }
